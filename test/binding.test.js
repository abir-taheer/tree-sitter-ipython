...
describe('tree-sitter-ipython binding', () => {
  ...
  test('exports language name', () => {
    assert.strictEqual(IPython.name, 'ipython', 'should export name as "ipython"');
  });

  test('exports language object (not function)', () => {
    assert.strictEqual(typeof IPython.language, 'object', 'language should be an object, not a function');
    assert.notStrictEqual(typeof IPython.language, 'function', 'language should NOT be a function');
  });
  ...
  test('parses line magic', () => {
    ...
    assert.strictEqual(lineMagic.type, 'line_magic', 'should parse as line_magic');

    const nameNode = lineMagic.childForFieldName('name');
    assert.ok(nameNode, 'should have name field');
    assert.strictEqual(nameNode.text, 'matplotlib', 'magic name should be "matplotlib"');

    const argsNode = lineMagic.childForFieldName('arguments');
    assert.ok(argsNode, 'should have arguments field');
    assert.strictEqual(argsNode.text, 'inline', 'arguments should be "inline" without leading space');
  });
  ...
  test('parses cell magic', () => {
    ...
    assert.strictEqual(cellMagic.type, 'cell_magic', 'should parse as cell_magic');
  ...
  test('parses shell escape', () => {
    ...
    assert.strictEqual(shellEscape.type, 'shell_escape', 'should parse as shell_escape');

    const commandNode = shellEscape.childForFieldName('command');
    assert.ok(commandNode, 'should have command field');
    assert.strictEqual(commandNode.text, 'pip install pandas', 'command should be "pip install pandas"');
  });
...
`;
    ...
    assert.strictEqual(children[0].type, 'line_magic', 'first should be line_magic');
    assert.strictEqual(children[1].type, 'import_statement', 'second should be import_statement');
    assert.strictEqual(children[2].type, 'shell_escape', 'third should be shell_escape');
    assert.strictEqual(children[3].type, 'cell_magic', 'fourth should be cell_magic');
  });
  ...
  test('parses shell escape inside indented block', () => {
    ...
    assert.strictEqual(forStmt.type, 'for_statement', 'should be for_statement');
    ...
    assert.strictEqual(shellEscape.type, 'shell_escape', 'should have shell_escape in block');

    const commandNode = shellEscape.childForFieldName('command');
    assert.strictEqual(commandNode.text, 'echo $i', 'command should be "echo $i"');
  });
  ...
  test('query works on parsed tree', () => {
    ...
    const query = new Parser.Query(IPython, '(line_magic name: (magic_name) @name)');
    const matches = query.matches(tree.rootNode);

    assert.ok(matches.length > 0, 'should have matches');
    assert.strictEqual(matches[0].captures[0].node.text, 'timeit', 'should capture magic name');
  });
});
